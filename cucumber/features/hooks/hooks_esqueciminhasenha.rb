    ...
    #   end
      ...
      Before '@login_email' do
        @mohmal = "http://www.yopmail.com/en/"
        visit @mohmal
        find('#login').set(@n_email)
        find(:css, '.sbut').click
        sleep 1
        @text_email = execute_script ('return window.document.getElementsByClassName("bname b al_l")[0].innerHTML')
        puts(@text_email)
        end
